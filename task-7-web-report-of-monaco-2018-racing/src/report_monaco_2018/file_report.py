...
def build_report(folder, desc=False):
    ...
    racers_dict = {key: racers_dict[key] for key in sorted(racers_dict, reverse=desc)}

    return list(racers_dict.values())


# Ф-я виводу report
def print_report(list_racers, desc=False):
...
def driver_statistics(list_racers, driver):
